...
This vignette will show you the major steps and the workflow of Heterogeneity-seq. This will include calculation of previous RNA profiles, trajectory generation and applying two Heterogeneity-seq methods.
...
First, we load in the HetSeq package as well as the preprocessed Seurat object:
...
library(cowplot)
library(reshape2)
data <- readRDS("../../zenodo/vignette/scifate_reduced.rds")
```
...
gc_genes <- readRDS("../../zenodo/vignette/gc_genes.rds")
...
res_classify <- HetseqClassify(data, trajectories, score.name = "dex.percent.new", num_cores = 32)
...
res_classify_cc <- HetseqClassify(data, trajectories, score.name = "dex.percent.new", basefeatures = c("cc_umap1", "cc_umap2"), num_cores = 32)
PlotClassify(res_classify_cc, highlights=c("ZFP36L1", "TRAM1", "ACTN4", "SRGN", "G6PD"), highlights.cutoff = 0.70, label.cutoff = 0.7)
